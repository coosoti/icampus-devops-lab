  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "2.5rem 1.5rem 5rem" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <h1 className="display-heading" style={{ fontSize: "1.8rem", color: "var(--text-primary)", marginBottom: "0.4rem" }}>
          Add project
        </h1>
      ...
      {success && (
        <div style={{ background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 8, padding: "0.75rem 1rem", marginBottom: "1.25rem", fontFamily: "var(--font-body)", fontSize: "0.85rem", color: "var(--accent-green)" }}>
          {success}
        </div>
      )}
      {error && (
        <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 8, padding: "0.75rem 1rem", marginBottom: "1.25rem", fontFamily: "var(--font-body)", fontSize: "0.85rem", color: "#f87171" }}>
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-section">
          ...
          <div className="form-grid">
            ...
            <div className="form-field" style={{ flexDirection: "row", alignItems: "center", gap: "0.75rem" }}>
              <input id="featured" type="checkbox" checked={form.featured} onChange={e => update("featured", e.target.checked)} style={{ width: 16, height: 16, accentColor: "var(--accent-blue)" }} />
          ...
          {coverImage ? (
            ...
            >
              {coverUploading ? (
                ...
                <>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="1.5" style={{ margin: "0 auto 0.5rem" }}><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
        ...
        <div className="form-section">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "0.75rem", marginBottom: "1rem" }}>
            {gallery.map((item, index) => (
              <div key={index} style={{ border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden", background: "var(--bg-secondary)" }}>
                <div style={{ aspectRatio: "16/9", position: "relative", background: "var(--bg-secondary)" }}>
                  {item.uploading ? (
                    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "var(--text-muted)" }}>Uploading...</span>
                    </div>
                  ) : (
                    <img src={item.url} alt={item.caption || `Gallery ${index + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  )}
                </div>
                <div style={{ padding: "0.4rem" }}>
                  <input value={item.caption} onChange={e => updateCaption(index, e.target.value)} placeholder="Caption (optional)" className="input" style={{ fontSize: "0.72rem", padding: "0.3rem 0.5rem" }} />
                </div>
                <button type="button" onClick={() => removeGalleryItem(index)}
                  style={{ width: "100%", background: "rgba(239,68,68,0.1)", border: "none", borderTop: "1px solid var(--border)", color: "#f87171", fontFamily: "var(--font-mono)", fontSize: "0.65rem", padding: "0.3rem", cursor: "pointer" }}>
                  Remove
                </button>
              </div>
            ))}
            ...
            <div onClick={() => galleryRef.current?.click()}
              ...
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="1.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            {[
              ...
              <div key={key} className="form-field">
                ...
                <textarea id={key} className="input" required={required} value={form[key]} onChange={e => update(key, e.target.value)} placeholder={placeholder} style={{ minHeight: 100 }} />
              </div>
            ))}
            <div className="form-field">
              <label className="label" htmlFor="failureScenarios">Failure scenarios</label>
              <textarea id="failureScenarios" className="input" value={form.failureScenarios} onChange={e => update("failureScenarios", e.target.value)} placeholder={"DB connection exhaustion: RDS max_connections hit. Resolution: added PgBouncer...\nFailed ECS deployment: bad image pushed to ECR. Resolution: re-tagged last good image..."} style={{ minHeight: 120 }} />
              <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "var(--text-muted)", marginTop: "0.25rem" }}>One scenario per line. Format: Title: description. Resolution: fix.</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
          ...
          <button type="button" className="btn-ghost" onClick={() => { setForm(EMPTY_FORM); setCoverImage(null); setGallery([]); setSelectedTagIds([]); }}>
            Reset form
          </button>
        </div>
      </form>
