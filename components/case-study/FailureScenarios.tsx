...
export default function FailureScenarios({ scenarios }: FailureScenariosProps) {
  ...
  return (
    <section style={{ marginBottom: "3rem" }} aria-label="Failure scenarios">
      <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "0.4rem" }}>
        <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#ef4444", flexShrink: 0 }} aria-hidden="true" />
      ...
      <div role="note" style={{ border: "1px solid rgba(239,68,68,0.25)", background: "rgba(239,68,68,0.06)", borderRadius: 8, padding: "0.7rem 1rem", marginBottom: "1.5rem" }}>
      ...
      {/* ol — semantically correct for ordered/numbered scenarios */}
      <ol style={{ display: "flex", flexDirection: "column", gap: "0.85rem", listStyle: "none", padding: 0, margin: 0 }}>
        {scenarios.map((scenario, index) => {
          ...
          return (
            <li key={index}>
              <article style={{ background: "var(--card-bg)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden" }}> {/* was var(--bg-card) */}
                {/* Header */}
                <header style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.65rem 1rem", borderBottom: "1px solid var(--border)", background: "rgba(255,255,255,0.02)" }}>
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "var(--text-muted)" }} aria-hidden="true">
                ...
                <div style={{ padding: "0.85rem 1rem", display: "flex", flexDirection: "column", gap: "0.65rem" }}>
                  ...
                  {resolution && (
                    <div style={{ display: "flex", gap: "0.6rem", paddingTop: "0.25rem" }}>
                      <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--accent-cyan)", flexShrink: 0, marginTop: 6 }} aria-hidden="true" /> {/* was --accent-green */}
                      <div>
                        <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: "var(--accent-cyan)", textTransform: "uppercase", letterSpacing: "0.08em" }}> {/* was --accent-green */}
                          Resolution
                        </span>
